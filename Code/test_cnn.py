...
#             dim = (width, height)
#             imgs = np.zeros((NUM_IMAGES, height, width))
...
#
# cnn.compile(loss=keras.losses.categorical_crossentropy, optimizer=adam_v2.Adam(learning_rate=0.001), metrics=['accuracy'])
# cnn.summary()
#
...
x_valid = x_valid.reshape(-1, height, width, 3)
...
cnn = Sequential()
cnn.add(Conv2D(32, kernel_size=(3, 3), activation='linear', padding='same', input_shape=(height, width, 3)))
cnn.add(LeakyReLU(alpha=0.1))
cnn.add(MaxPooling2D((2, 2), padding='same'))
cnn.add(Dropout(0.25))
cnn.add(Conv2D(64, (3, 3), activation='linear', padding='same'))
